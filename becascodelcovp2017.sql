...
-- Script date 21/12/17 8:29:22 AM
...
--
-- Definition for table RESULTADOS_POSTULACIONES
--
DROP TABLE IF EXISTS RESULTADOS_POSTULACIONES;
CREATE TABLE RESULTADOS_POSTULACIONES (
  RUT VARCHAR(50) NOT NULL,
  ESTADO VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_spanish2_ci NOT NULL,
  GLOSA VARCHAR(2000) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL
)
ENGINE = INNODB
AVG_ROW_LENGTH = 555
CHARACTER SET latin1
...
--
-- Definition for table RUT_PERMITIDOS
--
DROP TABLE IF EXISTS RUT_PERMITIDOS;
CREATE TABLE RUT_PERMITIDOS (
  RUT VARCHAR(250) NOT NULL,
  RUT_PERSONA VARCHAR(250) DEFAULT NULL,
...
--
-- Definition for procedure sproc_show_max_memory
--
DROP PROCEDURE IF EXISTS sproc_show_max_memory$$
CREATE DEFINER = 'root'@'127.0.0.1'
PROCEDURE sproc_show_max_memory( OUT max_memory DECIMAL(7,4))
BEGIN
...
--
-- Definition for function digitoVerificador
--
DROP FUNCTION IF EXISTS digitoVerificador$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION digitoVerificador(rut INT UNSIGNED)
...
--
-- Definition for view BUSCAR_TICKET
--
DROP VIEW IF EXISTS BUSCAR_TICKET CASCADE;
CREATE OR REPLACE 
	DEFINER = 'root'@'localhost'
VIEW BUSCAR_TICKET
AS
	select `POSTULACIONES_WEB`.`RUT_TRABAJADOR` AS `RUT`,`ESTADO_BECA`.`ESTADO_BECA` AS `ESTADO`,`ESTADO_BECA`.`ESTADO_BECA_GLOSA` AS `GLOSA` from (`POSTULACIONES_WEB` join `ESTADO_BECA` on((`POSTULACIONES_WEB`.`IDESTADOBECA` = `ESTADO_BECA`.`IDESTADOBECA`)));
...
VIEW existe_codigo
AS
	select `POSTULACIONES_WEB`.`IDESTADOBECA` AS `IDESTADOBECA`,`VIVIENDA`.`IDVIVIENDA` AS `id`,'VIVIENDA' AS `tipo`,concat(`POSTULACIONES_WEB`.`IDTIPOBECA`,'2015',`POSTULACIONES_WEB`.`RUT_TRABAJADOR`) AS `ticket` from (`POSTULACIONES_WEB` left join `VIVIENDA` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `VIVIENDA`.`IDPOSTULACION`))) where ((`POSTULACIONES_WEB`.`IDESTADOBECA` > 1) and (`VIVIENDA`.`IDVIVIENDA` is not null)) union all select `POSTULACIONES_WEB`.`IDESTADOBECA` AS `IDESTADOBECA`,`EDUCACION`.`IDEDUCACION` AS `id`,'EDUCACION' AS `tipo`,concat(`POSTULACIONES_WEB`.`IDTIPOBECA`,'2015',`POSTULACIONES_WEB`.`RUT_TRABAJADOR`) AS `ticket` from (`POSTULACIONES_WEB` left join `EDUCACION` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `EDUCACION`.`IDPOSTULACION`))) where ((`POSTULACIONES_WEB`.`IDESTADOBECA` > 1) and (`EDUCACION`.`IDEDUCACION` is not null));
...
AS
	select `POSTULACIONES_WEB`.`IDPOSTULACION` AS `IDPOSTULACION`,`POSTULACIONES_WEB`.`RENTA_TRABAJADOR` AS `RENTA_TRABAJADOR`,`POSTULACIONES_WEB`.`INTEGRANTES` AS `INTEGRANTES`,`EDUCACION`.`IDEDUCACION` AS `IDEDUCACION`,`EDUCACION`.`PROMEDIONOTAS_POSTULANTE` AS `PROMEDIONOTAS_POSTULANTE`,`EDUCACION`.`ENSENA_POSTULANTE` AS `ENSENA_POSTULANTE`,`EDUCACION`.`ANTENSENA_POSTULANTE` AS `ANTENSENA_POSTULANTE`,`RUT_PERMITIDOS`.`PUNTAJE` AS `RENTA_TRABAJADOR_BASE`,`POSTULACIONES_WEB`.`RUT_TRABAJADOR` AS `RUT_TRABAJADOR`,`RUT_PERMITIDOS`.`RUT_PERSONA` AS `RUT_PERSONA` from ((`POSTULACIONES_WEB` join `EDUCACION` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `EDUCACION`.`IDPOSTULACION`))) left join `RUT_PERMITIDOS` on((`POSTULACIONES_WEB`.`RUT_TRABAJADOR` = `RUT_PERMITIDOS`.`RUT`)));
...
AS
	select `COMUNAS`.`COMUNA_NOMBRE` AS `COMUNA_NOMBRE`,`REGIONES`.`REGION_NOMBRE` AS `REGION_NOMBRE`,`ESTADO_BECA`.`ESTADO_BECA` AS `ESTADO_BECA`,`POSTULACIONES_WEB`.`IDPOSTULACION` AS `IDPOSTULACION`,`POSTULACIONES_WEB`.`IDESTADOBECA` AS `IDESTADOBECA`,`POSTULACIONES_WEB`.`IDTIPOBECA` AS `IDTIPOBECA`,`POSTULACIONES_WEB`.`RUT_EMPRESA` AS `RUT_EMPRESA`,`POSTULACIONES_WEB`.`DV_EMPRESA` AS `DV_EMPRESA`,`POSTULACIONES_WEB`.`TIPO_EMPRESA` AS `TIPO_EMPRESA`,`POSTULACIONES_WEB`.`RAZONSOCIAL` AS `RAZONSOCIAL`,`POSTULACIONES_WEB`.`CORREO_EMPRESA` AS `CORREO_EMPRESA`,`POSTULACIONES_WEB`.`FONO_EMPRESA` AS `FONO_EMPRESA`,`POSTULACIONES_WEB`.`CONTRATO` AS `CONTRATO`,`POSTULACIONES_WEB`.`FECHA_CONTRATO` AS `FECHA_CONTRATO`,`POSTULACIONES_WEB`.`DIVISION` AS `DIVISION`,`POSTULACIONES_WEB`.`RUT_TRABAJADOR` AS `RUT_TRABAJADOR`,`POSTULACIONES_WEB`.`DV_TRABAJADOR` AS `DV_TRABAJADOR`,`POSTULACIONES_WEB`.`NOMBRE_TRABAJADOR` AS `NOMBRE_TRABAJADOR`,`POSTULACIONES_WEB`.`PATERNO_TRABAJADOR` AS `PATERNO_TRABAJADOR`,`POSTULACIONES_WEB`.`MATERNO_TRABAJADOR` AS `MATERNO_TRABAJADOR`,`POSTULACIONES_WEB`.`ESTADO_CIVIL` AS `ESTADO_CIVIL`,`POSTULACIONES_WEB`.`FECHA_NACIMIENTO` AS `FECHA_NACIMIENTO`,`POSTULACIONES_WEB`.`SEXO` AS `SEXO`,`POSTULACIONES_WEB`.`DIRECCION` AS `DIRECCION`,`POSTULACIONES_WEB`.`NUM_DIRECCION` AS `NUM_DIRECCION`,`POSTULACIONES_WEB`.`DEPTO_DIRECCION` AS `DEPTO_DIRECCION`,`POSTULACIONES_WEB`.`VILLA_DIRECCION` AS `VILLA_DIRECCION`,`POSTULACIONES_WEB`.`CORREO_TRABAJADOR` AS `CORREO_TRABAJADOR`,`POSTULACIONES_WEB`.`FONO_TRABAJADOR` AS `FONO_TRABAJADOR`,`POSTULACIONES_WEB`.`CELU_TRABAJADOR` AS `CELU_TRABAJADOR`,`POSTULACIONES_WEB`.`REGION_TRABAJADOR` AS `REGION_TRABAJADOR`,`POSTULACIONES_WEB`.`COMUNA_TRABAJADOR` AS `COMUNA_TRABAJADOR`,`POSTULACIONES_WEB`.`RENTA_TRABAJADOR` AS `RENTA_TRABAJADOR`,`POSTULACIONES_WEB`.`INTEGRANTES` AS `INTEGRANTES`,`VIVIENDA`.`IDVIVIENDA` AS `IDVIVIENDA`,`VIVIENDA`.`IDPONDVIVIENDA` AS `IDPONDVIVIENDA`,`VIVIENDA`.`ACEPTA_VIVIENDA` AS `ACEPTA_VIVIENDA`,`VIVIENDA`.`DESTINO_FONDO` AS `DESTINO_FONDO`,`VIVIENDA`.`VIVIENDA_DIRECCION` AS `VIVIENDA_DIRECCION`,`VIVIENDA`.`VIVIENDA_NUMERO` AS `VIVIENDA_NUMERO`,`VIVIENDA`.`VIVIENDA_DEPTO` AS `VIVIENDA_DEPTO`,`VIVIENDA`.`VIVIENDA_VILLA` AS `VIVIENDA_VILLA`,`VIVIENDA`.`VIVIENDA_COMUNA` AS `VIVIENDA_COMUNA`,`VIVIENDA`.`VIVIENDA_REGION` AS `VIVIENDA_REGION`,`VIVIENDA`.`ARCHIVO_CONTRATO` AS `ARCHIVO_CONTRATO`,`VIVIENDA`.`ARCHIVO_LIQ1` AS `ARCHIVO_LIQ1`,`VIVIENDA`.`ARCHIVO_LIQ2` AS `ARCHIVO_LIQ2`,`VIVIENDA`.`ARCHIVO_LIQ3` AS `ARCHIVO_LIQ3`,`VIVIENDA`.`ARCHIVO_PROMESA` AS `ARCHIVO_PROMESA`,`VIVIENDA`.`ARCHIVO_PROPIEDAD` AS `ARCHIVO_PROPIEDAD`,`VIVIENDA`.`ARCHIVO_DEUDA` AS `ARCHIVO_DEUDA`,`VIVIENDA`.`ARCHIVO_DECLARACION` AS `ARCHIVO_DECLARACION`,`COMUNAS1`.`COMUNA_NOMBRE` AS `COMUNA_VIVIENDA`,`REGIONES1`.`REGION_NOMBRE` AS `REGION_VIVIENDA` from ((((((`POSTULACIONES_WEB` join `VIVIENDA` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `VIVIENDA`.`IDPOSTULACION`))) left join `COMUNAS` on((`POSTULACIONES_WEB`.`COMUNA_TRABAJADOR` = `COMUNAS`.`IDCOMUNAS`))) left join `REGIONES` on((`POSTULACIONES_WEB`.`REGION_TRABAJADOR` = `REGIONES`.`IDREGIONES`))) join `ESTADO_BECA` on((`POSTULACIONES_WEB`.`IDESTADOBECA` = `ESTADO_BECA`.`IDESTADOBECA`))) join `COMUNAS` `COMUNAS1` on((`VIVIENDA`.`VIVIENDA_COMUNA` = `COMUNAS1`.`IDCOMUNAS`))) left join `REGIONES` `REGIONES1` on((`VIVIENDA`.`VIVIENDA_REGION` = `REGIONES1`.`IDREGIONES`)));
...
--
-- Definition for view listar_masivo_ponderacion
--
DROP VIEW IF EXISTS listar_masivo_ponderacion CASCADE;
CREATE OR REPLACE 
	DEFINER = 'root'@'localhost'
VIEW listar_masivo_ponderacion
AS
	select `POSTULACIONES_WEB`.`IDPOSTULACION` AS `IDPOSTULACION`,`POSTULACIONES_WEB`.`RENTA_TRABAJADOR` AS `RENTA_TRABAJADOR`,`POSTULACIONES_WEB`.`INTEGRANTES` AS `INTEGRANTES`,`EDUCACION`.`IDEDUCACION` AS `IDEDUCACION`,`EDUCACION`.`PROMEDIONOTAS_POSTULANTE` AS `PROMEDIONOTAS_POSTULANTE`,`EDUCACION`.`ENSENA_POSTULANTE` AS `ENSENA_POSTULANTE`,`EDUCACION`.`ANTENSENA_POSTULANTE` AS `ANTENSENA_POSTULANTE`,`RUT_PERMITIDOS`.`PUNTAJE` AS `RENTA_TRABAJADOR_BASE`,`POSTULACIONES_WEB`.`RUT_TRABAJADOR` AS `RUT_TRABAJADOR`,`RUT_PERMITIDOS`.`RUT_PERSONA` AS `RUT_PERSONA`,`EDUCACION`.`IDPONDEDUCACION` AS `IDPONDEDUCACION` from ((`POSTULACIONES_WEB` join `EDUCACION` on((`POSTULACIONES_WEB`.`IDPOSTULACION` = `EDUCACION`.`IDPOSTULACION`))) left join `RUT_PERMITIDOS` on((`POSTULACIONES_WEB`.`RUT_TRABAJADOR` = `RUT_PERMITIDOS`.`RUT`)));
